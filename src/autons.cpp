...
std::string red_left_winpoint(bool calibrate, auto_variation var, bool get_name) {
   ...
   assembly.tongue_piston.toggle();
   chassis.drive_distance(11,{.max_voltage = 8, .timeout = 1500});
   wait(0.3,sec);
   chassis.turn_to_angle(-45, {.timeout = 650});
   chassis.drive_distance(44.25, {.timeout = 850});
   ...
   chassis.drive_distance(-10, {.max_voltage = 4, .timeout = 1000, .heading = 270, .wait = false});
...
std::string red_left_elim(bool calibrate, auto_variation var, bool get_name) {
   ...
   wait(0.3,sec);
   chassis.turn_to_angle(-45, {.timeout = 650});
   chassis.drive_distance(-17, {.timeout = 850});
...
std::string red_right_elim(bool calibrate, auto_variation var, bool get_name) {  
   ...
   odom_constants();
   chassis.turn_to_angle(107, {.timeout = 200});
   assembly.ptoPiston.close();
   assembly.intake_motor.spin(forward,12,volt);
   assembly.outtake_motor.spin(forward,12,volt);
   chassis.drive_distance(18);
   assembly.tongue_piston.toggle();
   chassis.drive_distance(6,{.max_voltage = 7, .timeout = 400});
   wait(0.3,sec);
   assembly.tongue_piston.toggle();
   chassis.turn_to_angle(225, {.timeout = 600});
   chassis.drive_distance(31,{.timeout = 800});
   ...
   chassis.turn_to_angle(270, {.timeout = 200});
   chassis.drive_distance(-24, {.heading = 270});
   chassis.drive_distance(-10, {.max_voltage = 5, .timeout = 1000, .heading = 270, .wait = false});
   assembly.ptoPiston.open();
   // wing block
   wait(2,sec);
   assembly.tongue_piston.toggle();
   chassis.drive_distance(18, {.heading = 240,.timeout = 750});
   chassis.drive_distance(-18, {.heading=300,.timeout = 1000});
   chassis.turn_to_angle(273, {.timeout = 300});
   assembly.wing_piston.toggle();
   wait(0.1,sec);
   chassis.drive_distance(-30, {.min_voltage = 12});
